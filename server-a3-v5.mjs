...
const agentVersion = "2026.09.15-dfe.5";
const mimeTypes = {
...
try {
  ...
  try {
    ...
    const labels = ["Falha Schannel/curl", "Falha TLS/SEFAZ", "O curl.exe"];
    const positions = labels.map(label => raw.indexOf(label)).filter(position => position >= 0);
    if (positions.length) {
      const marker = Math.min(...positions);
      const detail = raw.slice(marker).replace(/\r?\n/g, " ").replace(/\s+/g, " ").replace(/\s+No linha:.*$/i, "").trim().slice(0, 700);
      throw new Error(detail);
    }
    throw new Error("Falha ao acessar a SEFAZ com o certificado A3.");
  }
  let result;
