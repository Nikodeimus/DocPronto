@echo off
setlocal
title DocPronto - Agente de Certificados V9
color 0F

echo.
echo ============================================
echo       DOCPRONTO - AGENTE DE CERTIFICADOS V9
echo ============================================
echo.
echo Esta versao corrige a consulta CT-e e preserva
echo o ultimo NSU devolvido pela SEFAZ.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { Invoke-WebRequest -UseBasicParsing 'https://raw.githubusercontent.com/Nikodeimus/DocPronto/main/server-cert-v8.mjs?v=cert-9' -OutFile '%AGENT_DIR%\server-cert-v9.mjs' } catch { Write-Host $_.Exception.Message; exit 1 }"
if errorlevel 1 (
...
start "DocPronto - Agente V9 (mantenha aberto)" /D "%AGENT_DIR%" cmd /k "set PORT=4174&& node server-cert-v9.mjs"
timeout /t 3 /nobreak >nul
start "" "https://nikodeimus.github.io/DocPronto/hub/?v=cert-9"

echo.
echo Agente de certificados V9 iniciado. Mantenha a nova janela aberta.
